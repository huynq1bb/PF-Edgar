...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (!name || !triggerValue) {
    return { error: "Rule name và Trigger value (sản phẩm chính) là bắt buộc." };
  }
  ...
  if (!triggerGid) {
    return { error: "Trigger value không hợp lệ. Nhập link sản phẩm hoặc GID." };
  }
...
export default function CrossSellEdit() {
  ...
  const [suggest1, setSuggest1] = useState(rule.suggestedProductId1 ?? "");
  const [suggest2, setSuggest2] = useState(rule.suggestedProductId2 ?? "");
  const [suggest3, setSuggest3] = useState(rule.suggestedProductId3 ?? "");
  ...
  // Khi mở rule đã lưu: load preview từ 4 GID đã lưu để preview hiển thị đúng dữ liệu lúc lưu (không đổi so với lúc tạo/save).
  useEffect(() => {
    if (rule.triggerValue) fetcherTrigger.load(`/app/cross-sell/product-preview?product=${encodeURIComponent(rule.triggerValue)}`);
    if (rule.suggestedProductId1) fetcher1.load(`/app/cross-sell/product-preview?product=${encodeURIComponent(rule.suggestedProductId1)}`);
    if (rule.suggestedProductId2) fetcher2.load(`/app/cross-sell/product-preview?product=${encodeURIComponent(rule.suggestedProductId2)}`);
    if (rule.suggestedProductId3) fetcher3.load(`/app/cross-sell/product-preview?product=${encodeURIComponent(rule.suggestedProductId3)}`);
  }, [rule.id]);

  return (
    <s-page heading={`Edit: ${rule.name}`}>
      <s-section>
        <Form method="post">
          <s-stack direction="block" gap="base">
            ...
            <div>
              ...
              <input id="name" name="name" type="text" required defaultValue={rule.name} style={inputStyle} />
            </div>
            ...
            <s-stack direction="inline" gap="base" style={{ marginTop: "16px" }}>
              ...
              <Form method="post">
                ...
                <button type="submit" style={{ padding: "8px 16px", fontSize: "14px", color: "#c00", background: "transparent", border: "1px solid #c00", borderRadius: "6px", cursor: "pointer" }}>
                  Delete rule
                </button>
              </Form>
            </s-stack>
